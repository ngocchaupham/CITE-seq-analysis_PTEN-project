...
BCObj <- BC_WT
Idents(BCObj) <- "wsnn_res.0.6"
# check Seurat object version (scRNA-seq matrix extracted differently in Seurat v4/v5)
seurat_package_v5 <- isFALSE('counts' %in% names(attributes(BCObj[["RNA"]])));
print(sprintf("Seurat object %s is used", ifelse(seurat_package_v5, "v5", "v4")))

# extract scaled scRNA-seq matrix
scRNAseqData_scaled <- if (seurat_package_v5) as.matrix(BCObj[["RNA"]]$scale.data) else as.matrix(BCObj[["RNA"]]@scale.data)
...
# merge by cluster
cL_resutls <- do.call("rbind", lapply(unique(BCObj@meta.data$seurat_clusters), function(cl){
    es.max.cl = sort(rowSums(es.max[ ,rownames(BCObj@meta.data[BCObj@meta.data$seurat_clusters==cl, ])]), decreasing = !0)
    head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(BCObj@meta.data$seurat_clusters==cl)), 10)
}))
...
```{r}
BCObj@meta.data$sctype_classification = ""
for(j in unique(sctype_scores$cluster)){
  cl_type = sctype_scores[sctype_scores$cluster==j,]; 
  BCObj@meta.data$sctype_classification[BCObj@meta.data$seurat_clusters == j] = as.character(cl_type$type[1])
}

DimPlot(BCObj, reduction = "wnn.umap", label = TRUE, repel = TRUE, group.by = 'sctype_classification')        
...
DimPlot(testObj, reduction = "umap", label = TRUE, repel = TRUE, cols = ccolss)+ gggr
```
