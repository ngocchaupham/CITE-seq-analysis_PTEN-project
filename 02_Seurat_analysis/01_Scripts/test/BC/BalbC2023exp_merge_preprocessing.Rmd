---
title: "Merge_BalbC_2023"
output: html_document
date: "2024-01-19"
editor_options: 
  chunk_output_type: console
---
# Merge 2023 experiments
Try to merge pool 1/2/3 of march 2023 
...
```{r}
#loading object
load( file = "/mnt/NASBIOINFO_CP/LALT/BIOINFO/PTEN/03_Analysis/Analysis2024/RObject/20230320.Robj")
Obj20230320 <- Seurat

load( file = "/mnt/NASBIOINFO_CP/LALT/BIOINFO/PTEN/03_Analysis/Analysis2024/RObject/20230327.Robj")
Obj20230327 <- Seurat

load( file = "/mnt/NASBIOINFO_CP/LALT/BIOINFO/PTEN/03_Analysis/Analysis2024/RObject/20230303.Robj")
Obj20230303 <- Seurat
```
...
## Rename
```{r}
#premier level d'annotation a preciser avec late early en fct de l'analyse
#rename 230320
...
#save(bc.combined,file = "/mnt/NASBIOINFO_CP/LALT/BIOINFO/PTEN/03_Analysis/Analysis2024/RObject/merge_2023exp.Robj")
...
Identification subset CD4-CD8- CD25-
```{r}
...
# sur les 20 premieres composantes
bc.combined <- RunPCA(object = bc.combined,
                    assay = "RNA",
                    verbose = FALSE, #if TRUE print the top genes for each PC
                    seed.use = 1234,
...
ElbowPlot(bc.combined, ndims = 20, reduction = "pca")
# # Determine percent of variation associated with each PC
pct <- bc.combined[["pca"]]@stdev / sum(bc.combined[["pca"]]@stdev) * 100
# # Calculate cumulative percents for each PC
cumu <- cumsum(pct)
...
bc.combined <- FindClusters(object = bc.combined, 
                          ...
                          verbose = FALSE,
                          random.seed = 1234)
bc.combined <- RunUMAP(object = bc.combined, reduction = "pca", seed.use = 1234, dims = 1:14)
...
bc.combined <- FindClusters(object = bc.combined, 
                          ...
                          verbose = FALSE,
                          random.seed = 1234)
bc.combined <- RunUMAP(object = bc.combined, reduction = "pca", seed.use = 1234, dims = 1:17)
...
```{r}
# sur les 50 premieres composantes
bc.combined <- RunPCA(object = bc.combined,
                    assay = "RNA",
                    verbose = FALSE, #if TRUE print the top genes for each PC
                    seed.use = 1234,
...
bc.combined <- FindClusters(object = bc.combined, 
                          ...
                          verbose = FALSE,
                          random.seed = 1234)
bc.combined <- RunUMAP(object = bc.combined, reduction = "pca", seed.use = 1234, dims = 1:42)
Idents(bc.combined) <- "annotation"
ggplotly(DimPlot(bc.combined, group.by = "annotation"))
FeaturePlot(bc.combined, reduction = "umap", features = c("adt_CD25","Top2a","Mki67","adt_CD8","adt_CD4","adt_CD3","adt_CD62L","adt_CD44","adt_CCR7","Cpa3","Nkg7","adt_TCRgd", "percent.mt"), cells = WhichCells(bc.combined, idents = "Thymus-WT")) & scale_colour_gradientn(colours = rev(viridis::magma(10)))


ggplotly(DimPlot(bc.combined, group.by = "Phase"))
...
bc.combined.cc <- ProjectDim(object = bc.combined.cc,
                        nfeatures.print = 10,
                        dims.print = 1:50)
...
DimPlot(bc.combined.cc,reduction = "pca",dims = 41:42,group.by = "MULTI_ID") 


#Clustering and Visualization
#Dim = 42 
bc.combined.cc <- FindNeighbors(object = bc.combined.cc, 
                           dims = 1:42, 
                           verbose = FALSE, 
                           reduction = "pca")

bc.combined.cc <- FindClusters(object = bc.combined.cc, 
                          resolution =1 ,
                          verbose = FALSE,
                          random.seed = 1234)
bc.combined.cc <- RunUMAP(object = bc.combined.cc, reduction = "pca", seed.use = 1234, dims = 1:42)


Idents(bc.combined.cc) <- "annotation"
ggplotly(DimPlot(bc.combined.cc, group.by = "annotation", shape.by = "orig.ident"))

#visualize cell cycle phases
DimPlot(bc.combined, group.by = "Phase")
DimPlot(bc.combined.cc, group.by = "Phase")

Idents(bc.combined) <- "Phase"
RidgePlot(bc.combined, features = c("Pcna", "Top2a", "Mcm6", "Mki67"), ncol = 2)

##visualize some important features
DefaultAssay(bc.combined.cc)
FeaturePlot(bc.combined.cc,features = c("adt_CD25","adt_CD8","adt_CD4","adt_CD3","adt_CD62L","adt_CD44","Myc"))& scale_colour_gradientn(colours = rev(viridis::magma(10)))

FeaturePlot(bc.combined.cc,features = c("adt_CD25","adt_CD8","adt_CD4","adt_CD3","adt_CD62L","adt_CD44","Mki67","Myc","percent.mt"), cells = WhichCells(bc.combined.cc, idents = "Thymus-WT")) & scale_colour_gradientn(colours = rev(viridis::magma(10)))

FeaturePlot(bc.combined.cc,features = c("S100a8","Foxp3","Klrb1c","Nkg7")) & scale_colour_gradientn(colours = rev(viridis::magma(10)))
...
Idents(bc.combined.cc) <- factor(Idents(bc.combined.cc), levels = c("Thymus-51","Spleen-51","Thymus-83","Spleen-83","Thymus-47","Spleen-47","Thymus-85","Spleen-85","Thymus-84","Spleen-84","Thymus-39","Spleen-39",  "Thymus-52","Spleen-52","Thymus-70","Spleen-70", "Thymus-80","Spleen-80","Thymus-53", "Spleen-53","Thymus-69", "Spleen-69"))
VlnPlot(bc.combined.cc, features = "nFeature_RNA", pt.size = 0, cols = c("#9AC4F8","#9AC4F8","#9AC4F8","#9AC4F8","#7EB488","#7EB488","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#BB0A21","#BB0A21","#BB0A21","#BB0A21","#BB0A21","#BB0A21"))+geom_hline(yintercept = 2000, linetype ="dashed", size = 1, color="black")+
...
Idents(bc.combined.cc)<-"annotation"
Idents(bc.combined.cc) <- factor(Idents(bc.combined.cc), levels = c("Thymus-WT", "Spleen-WT","Thymus-PreTum early","Spleen-PreTum early","Thymus-PreTum late","Spleen-PreTum late","Thymus-Tum","Spleen-Tum" ))
VlnPlot(bc.combined.cc, features = "nFeature_RNA", pt.size = 0, )+geom_hline(yintercept = 2000, linetype ="dashed", size = 1, color="black")+
  stat_summary(fun = "mean",
           geom = "crossbar",
           color = 'firebrick1', show.legend = F)

VlnPlot(bc.combined.cc, features = "Myc", pt.size = 0, cols = c("#9AC4F8","#9AC4F8","#9AC4F8","#9AC4F8","#7EB488","#7EB488","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#fac341","#BB0A21","#BB0A21","#BB0A21","#BB0A21","#BB0A21","#BB0A21"))+geom_hline(yintercept = 2000, linetype ="dashed", size = 1, color="black")+
  stat_summary(fun = "mean",
           geom = "crossbar",
           color = 'firebrick1', show.legend = F)
...
```{r}
ggplotly(DimPlot(bc.combined.cc, reduction = "wnn.umap", group.by = "wsnn_res.1"))
ggplotly(DimPlot(bc.combined.cc, reduction = "wnn.umap", group.by = "wsnn_res.0.1"))
DimPlot(bc.combined.cc, reduction = "wnn.umap", cells = WhichCells(bc.combined.cc, idents = c("2","13")))
Idents(bc.combined.cc) <- "wsnn_res.1"
M <- FindMarkers(bc.combined.cc, ident.1 = "2", ident.2 = "13")
datatable(M)

Idents(bc.combined.cc) <- "annotation"
FeaturePlot(bc.combined.cc, reduction = "wnn.umap", features = c("adt_CD25","adt_CD8","adt_CD4","adt_CD3","Klrd1","Nkg7","Gm525","Ly6d","percent.mt"), cells = WhichCells(bc.combined.cc, idents = "Thymus-WT")) & scale_colour_gradientn(colours = rev(viridis::magma(10)))

Idents(bc.combined.cc) <- "wsnn_res.1"
FeaturePlot(bc.combined.cc, reduction = "wnn.umap", features = c("adt_CD25","adt_CD8","adt_CD4","adt_CD3","Klrd1","Nkg7","Gm525","Ly6d","percent.mt"), cells = WhichCells(bc.combined.cc, idents = c("2","13"))) & scale_colour_gradientn(colours = rev(viridis::magma(10)))
```
...
#WNN analysis (RNA+ADT) on unregressed objet

```{r}
#load(file = "/mnt/NASBIOINFO_CP/LALT/BIOINFO/PTEN/03_Analysis/Analysis2024/RObject/merge_2023exp.Robj")
DefaultAssay(bc.combined) <- 'RNA'
bc.combined <- NormalizeData(bc.combined) %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA()
ElbowPlot(bc.combined, ndims = 30, reduction = "pca")

DefaultAssay(bc.combined) <- 'ADT'
# we will use all ADT features for dimensional reduction
# we set a dimensional reduction name to avoid overwriting the 
VariableFeatures(bc.combined) <- rownames(bc.combined[["ADT"]])
bc.combined <- NormalizeData(bc.combined, normalization.method = 'CLR', margin = 2) %>% 
  ScaleData() %>% RunPCA(reduction.name = 'apca')
ElbowPlot(bc.combined, ndims = 13, reduction = "apca")

# Identify multimodal neighbors. These will be stored in the neighbors layers, 
bc.combined <- FindMultiModalNeighbors(
  bc.combined, reduction.list = list("pca", "apca"), 
  dims.list = list(1:29, 1:11), modality.weight.name = c("RNA.weight", "ADT.weight"))

bc.combined <- RunUMAP(bc.combined, nn.name = "weighted.nn", reduction.name = "wnn.umap", reduction.key = "wnnUMAP_", seed.use = 1234)
bc.combined <- FindClusters(bc.combined, graph.name = "wsnn", algorithm = 3, resolution = 1, verbose = FALSE, random.seed = 1234)

bc.combined <- RunUMAP(bc.combined, reduction = 'pca', dims = 1:29, assay = 'RNA', 
              reduction.name = 'rna.umap', reduction.key = 'rnaUMAP_', seed.use = 1234)
bc.combined <- RunUMAP(bc.combined, reduction = 'apca', dims = 1:11, assay = 'ADT', 
              reduction.name = 'adt.umap', reduction.key = 'adtUMAP_', seed.use = 1234)

#To compare 
p1 <- DimPlot(bc.combined, reduction = "rna.umap", group.by = "annotation") + ggtitle("RNA")
p2 <- DimPlot(bc.combined, reduction = "adt.umap", group.by = "annotation") + ggtitle("ADT")
p3 <- DimPlot(bc.combined, reduction = "wnn.umap", group.by = "annotation") + ggtitle("WNN")
p1 + p2 + p3 & NoLegend() & theme(plot.title = element_text(hjust = 0.5))

#ggplotly(DimPlot(bc.combined, reduction = "wnn.umap", group.by = "annotation", shape.by = "orig.ident"))

DefaultAssay(bc.combined) <- 'RNA'
Idents(bc.combined) <- "annotation"
FeaturePlot(bc.combined, reduction = "wnn.umap", features = c("adt_CD25","Il2ra","Mki67","adt_CD8","adt_CD4","adt_CD3","adt_CD62L","adt_CD69","Myc","adt_TCRgd","Rag1","percent.mt"), cells = WhichCells(bc.combined, idents = "Thymus-WT")) & scale_colour_gradientn(colours = rev(viridis::magma(10)))
```



```{r}
ggplotly(DimPlot(bc.combined, reduction = "wnn.umap"))
bc.combined <- JoinLayers(bc.combined)
M <- FindMarkers(bc.combined, ident.1 = "18", ident.2 = "2")
datatable(M)
DefaultAssay(bc.combined) <- 'RNA'
Idents(bc.combined) <- "annotation"
FeaturePlot(bc.combined, reduction = "wnn.umap", features = c("adt_CD25","adt_CD8","adt_CD4","adt_CD3","adt_CD62L","adt_CD44","Gm525","Ly6d","percent.mt"), cells = WhichCells(bc.combined, idents = "Thymus-WT")) & scale_colour_gradientn(colours = rev(viridis::magma(10)))
#save(bc.combined, file = "/mnt/NASBIOINFO_CP/LALT/BIOINFO/PTEN/03_Analysis/Analysis2024/RObject/merge_2023exp.Robj")
```
